...
class Bdtaskt1MaterialStoreModel extends Model
{
    ...
    public function __construct()
    {
        ...
        $this->hasReadAccess = $this->permission->method('wh_material_store', 'read')->access();
        $this->hasCreateAccess = $this->permission->method('wh_material_store', 'create')->access();
        $this->hasUpdateAccess = $this->permission->method('wh_material_store', 'update')->access();
        $this->hasDeleteAccess = $this->permission->method('wh_material_store', 'delete')->access();
    ...
    public function bdtaskt1m12_02_getAll($postData=null){
        ...
        $searchQuery = "";
        if($searchValue != ''){
           $searchQuery = " (wh_material_store.nameE like '%".$searchValue."%' OR wh_material_store.nameA like '%".$searchValue."%' ) ";
        }
        
        ## Fetch records
        $builder3 = $this->db->table('wh_material_store');
        $builder3->select("wh_material_store.*, branch.nameE as branch_name");
        if($searchValue != ''){
        ...
        $builder3->join('branch', 'branch.id=wh_material_store.branch_id','left');
    ...
    public function bdtaskt1m12_03_getMainStoreDetailsById($id){
        $data = $this->db->table('wh_material_store')->select('wh_material_store.*, branch.nameE as branch_name')           
                        ->join('branch', 'branch.id=wh_material_store.branch_id','left')
                        ->where( array('wh_material_store.id'=>$id) )
                        ->get()
                        ->getRowArray();
