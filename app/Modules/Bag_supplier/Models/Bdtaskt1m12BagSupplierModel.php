<?php namespace App\Modules\Bag_supplier\Models;
use CodeIgniter\Model;
use App\Libraries\Permission;

class Bdtaskt1m12BagSupplierModel extends Model
{
    ...
    public function __construct()
    {
        ...
        $this->hasCreateAccess = $this->permission->method('wh_bag_supplier', 'create')->access();
    ...
    public function bdtaskt1m12_01_getAllSupplier($postData=null){
        ...
        $searchQuery = "";
        if($searchValue != ''){
           $searchQuery = " (spl.nameE like '%".$searchValue."%' OR spl.ssn like '%".$searchValue."%' OR spl.phone like '%".$searchValue."%' OR spl.acc_head like '%".$searchValue."%') ";
        }
        ## Fetch records
        $builder3 = $this->db->table('wh_bag_supplier_information spl');
        $builder3->select("spl.*");
        if($searchValue != ''){
        ...
        foreach($records as $record ){ 
            ...
            $button = '';
            $button .= (!$this->hasReadAccess)?'':'<a href="javascript:void(0);" class="btn btn-info-soft btnC actionPreview mr-1 custool" title="'.$view.'" data-id="'.$record['id'].'"><i class="fas fa-eye"></i></a>';
