...
class Bdtaskt1m12SupplierModel extends Model
{
    ...
    public function bdtaskt1m12_01_getAllSupplier($postData=null){
        ...
        $searchQuery = "";
        if($searchValue != ''){
           $searchQuery = " (spl.nameE like '%".$searchValue."%' OR spl.nameA like '%".$searchValue."%' OR spl.phone like '%".$searchValue."%' OR spl.email like '%".$searchValue."%' OR spl.acc_head like '%".$searchValue."%') ";
        }
        if($postData['supplier_type'] != ''){
            if($searchQuery != ''){
                $searchQuery .= " AND ";
            }
           $searchQuery .= " ( spl.supplier_type = '".$postData['supplier_type']."' ) ";
        }
        ## Fetch records
        ...
        foreach($records as $record ){ 
            ...
            if ($record['supplier_type'] == 1) {
                $supplier_type = get_phrases(['local', 'purchase', 'supplier']);
            }else if ($record['supplier_type'] == 2) {
