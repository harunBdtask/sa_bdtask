<?php namespace App\Modules\Bag_store\Controllers;
use App\Modules\Bag_store\Views;
use CodeIgniter\Controller;
use App\Modules\Bag_store\Models\Bdtaskt1MaterialStoreModel;
...
class Bdtaskt1c1MaterialStore extends BaseController
{
    ...
    *--------------------------*/
    ...
    {
        $data['moduleTitle']= get_phrases(['bag','store']);
        $data['title']      = get_phrases(['store', 'list']);
        $data['isDTables']  = true;
        $data['module']     = "Bag_store";
        $data['page']       = "material_store/list";
        $data['setting']    = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_03_getRow('setting');

        $data['hasCreateAccess']        = $this->permission->method('wh_bag_store', 'create')->access();
        $data['hasPrintAccess']        = $this->permission->method('wh_bag_store', 'print')->access();
        $data['hasExportAccess']        = $this->permission->method('wh_bag_store', 'export')->access();
    ...
    *--------------------------*/
    ...
    { 
        $material = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_bag', array('store_id'=>$id));
        $material_receive = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_bag_receive', array('store_id'=>$id));
        $main_stock = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_bag_stock', array('store_id'=>$id, 'stock >'=>0 ));
        ...
        $data = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_06_Deleted('wh_bag_store', array('id'=>$id));
        // Store log data
        $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['material','store']), get_phrases(['deleted']), $id, 'wh_bag_store');
        //$MesTitle = get_phrases(['store', 'record']);
        if(!empty($data)){
            $response = array(
                'success'  =>true,
    ...
    *--------------------------*/
    ...
    { 
        ...
        if (! $this->validate($rules)) {
            ...
            if($action=='add'){
                $isExist = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_store', array('nameE'=>$this->request->getVar('nameE')));
                if(!empty($isExist) || !empty($isExist2) ){
                    ...
                    $result = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_01_Insert('wh_bag_store',$data);
                    if($result){
                        ...
                        // $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_02_Update('wh_bag_store',array('acc_head'=>$Coa), array('id'=>$result));

                        // Store log data
                        $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['material','store']), get_phrases(['created']), $result, 'wh_bag_store');
                         $response = array(
                            'success'  =>true,
                ...
                $store_row = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_store', array('id'=>$id));
                //$data['image'] = !empty($filePath)?$filePath:$old_image;
                $data['updated_by']          = session('id');
                $data['updated_date']        = date('Y-m-d H:i:s');

                $result = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_02_Update('wh_bag_store',$data, array('id'=>$id));
                if($result){
                    // $maxCoa = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_18_getLikeMaxData('acc_coa', '2000','HeadCode');
                    // $Coa = ((int)$maxCoa + 1);
                    // $coaData = [
                    ...
                    //     $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_01_Insert('acc_coa',$coaData);
                    //     $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_02_Update('wh_bag_store',array('acc_head'=>$Coa), array('id'=>$id));
                    // }else{
                    ...
                    // }
                    // Store log data
                    $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['material','store']), get_phrases(['updated']), $id, 'wh_bag_store');
                     $response = array(
                        'success'  =>true,
    ...
    *--------------------------*/
    ...
    { 
        $data = $this->bdtaskt1m12c4_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_store', array('id'=>$id));
        echo json_encode($data);
    }
