...
use App\Modules\Bag_store\Models\Bdtaskt1MaterialStockModel;
...
class Bdtaskt1c2MaterialStock extends BaseController
{
    ...
    *--------------------------*/
    public function index()
    {
        ...
        $data['hasPrintAccess']        = $this->permission->method('wh_bag_stock', 'print')->access();
        $data['hasExportAccess']        = $this->permission->method('wh_bag_stock', 'export')->access();
        
        $where = array('status'=>1,'branch_id'=>session('branchId') );

        $data['store_list']       = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_bag_store', $where);
    ...
    *--------------------------*/
    ...
    { 
        $data = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_06_Deleted('wh_bag_stock', array('id'=>$id));
    ...
    *--------------------------*/
    ...
    { 
        ...
        if (! $this->validate($rules)) {
            ...
            if($action=='add'){
                $isExist = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_stock', array('nameE'=>$this->request->getVar('nameE')));
                $isExist2 = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_stock', array('nameA'=>$this->request->getVar('nameA')));
                if(!empty($isExist) || !empty($isExist2) ){
                    ...
                    //$data['image'] = $filePath;
                    $result = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_01_Insert('wh_bag_stock',$data);
                    if($result){
                         $response = array(
                            'success'  =>true,
                ...
                $result = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_02_Update('wh_bag_stock',$data, array('id'=>$id));
                if($result){
                     $response = array(
                        'success'  =>true,
    ...
    *--------------------------*/
    ...
    { 
        $data = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_stock', array('id'=>$id));
        echo json_encode($data);
    }
