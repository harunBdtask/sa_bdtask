...
class Bdtaskt1m12ItemsModel extends Model
{
    ...
    public function __construct()
    {
        ...
        $this->hasReadAccess = $this->permission->method('wh_material_items', 'read')->access();
        $this->hasCreateAccess = $this->permission->method('wh_material_items', 'create')->access();
        $this->hasUpdateAccess = $this->permission->method('wh_material_items', 'update')->access();
        $this->hasDeleteAccess = $this->permission->method('wh_material_items', 'delete')->access();
    ...
    public function bdtaskt1m12_02_getAll($postData=null){
        ...
        if($cat_id != ''){
           ...
           $searchQuery .= " ( wh_material.cat_id = '".$cat_id."' ) ";
        }
        ## Fetch records
        $builder3 = $this->db->table('wh_material');
        $builder3->select("wh_material.*, store.nameE as store_name, wh_material_categories.nameE as cat_name, list_data.nameE as unit_name");
        if($searchQuery != ''){
           $builder3->where($searchQuery);
        }                 
        $builder3->join('wh_material_store store', 'store.id=wh_material.store_id','left');
        $builder3->join('wh_material_categories', 'wh_material_categories.id=wh_material.cat_id','left');
    ...
    public function bdtaskt1m12_03_getItemDetailsById($id){
        $data = $this->db->table('wh_material')->select('wh_material.*, store.nameE as store_name, ld.nameE as w_use, wh_material_categories.nameE as cat_name, list_data.nameE as unit_name')                        
                        ->join('wh_material_store store', 'store.id=wh_material.store_id','left')
                        ->join('wh_material_categories', 'wh_material_categories.id=wh_material.cat_id','left')
                        ->join('list_data', 'list_data.id=wh_material.unit_id','left')
                        ->join('list_data ld', 'ld.id=wh_material.where_use','left')
                        ->where( array('wh_material.id'=>$id) )
                        ->get()
                        ->getRowArray();
