...
class Bdtaskt1m12PurchaseOrderModel extends Model
{
    ...
    public function bdtaskt1m12_02_getAll($postData=null){
        ...
        $builder3->select("wh_bag_purchase.*, wmr.voucher_no as r_voucher, wq.id as q_id, wh_bag_supplier_information.nameE as supplier_name, GROUP_CONCAT(i.item_code) as item_codes, GROUP_CONCAT(i.nameE) as item_names");
        $builder3->join('wh_bag_purchase_details pd', 'pd.purchase_id=wh_bag_purchase.id','left');
        $builder3->join('wh_assets i', 'i.id=pd.item_id','left');
        ...
        $builder3->where( array('wmr.type'=>2) );
        if($searchQuery != ''){
    ...
    public function bdtaskt1m12_07_getPurchaseOrderItemDetailsById($purchase_id, $supplier_id){
        ...
        $data = $this->db->table('wh_bag_purchase_details')->select('wh_bag_purchase_details.*,list_data.nameE as unit_name')       
            ->join('wh_assets', 'wh_assets.id=wh_bag_purchase_details.item_id','left')
            ->join('list_data', 'list_data.id=wh_assets.unit_id','left')
