...
use App\Modules\Assets_purchase\Models\Bdtaskt1m12PurchaseOrderModel;
...
class Bdtaskt1m12c10PurchaseOrder extends BaseController
{
    ...
    *--------------------------*/
    public function bdtaskt1m12c10_03_addPurchaseOrder()
    {
        ...
        if (! $this->validate($rules)) {
            ...
            $po_remarks    = $this->request->getVar('po_remarks');
            ...
            if($action=='add'){
                ...
                if( !empty($isExist)  ){
                    ...
                    foreach($quatation_details as $key => $item){
                        $data2[] = array('purchase_id'=> $result, 'item_id'=> $item->item_id, 'qty'=> $item->qty, 'price'=> $item->price, 'total'=> $item->total, 'vat_applicable'=> $item->vat_applicable, 'vat_amount'=> $item->vat_amount, 'remarks'=> $po_remarks[$key] );
    ...
    *--------------------------*/
    ...
    public function bdtaskt1m12c10_07_approvePurchaseOrder(){
        ...
        if ($result) {
            ...
            $this->bdtaskt1m12c10_02_CmModel->bdtaskt1m1_02_Update('wh_bag_requisition',array('received'=>1,'type'=>2), array('id'=>$spr_id));
    ...
    *--------------------------*/
    ...
    { 
        ...
        $html = '<table class="table table-bordered w-100"><tr><th>'.get_phrases(['SL']).'</th><th class="text-center">'.get_phrases(['assets','name']).'</th><th class="text-right">'.get_phrases(['unit','price']).'</th><th class="text-right">'.get_phrases(['quantity']).'</th><th class="text-right">'.get_phrases(['total']).'</th><th>'.get_phrases(['remarks']).'</th></tr>';
        ...
        if ($quatation_details) {
            ...
            {
                $wh_bag = $this->bdtaskt1m12c10_02_CmModel->bdtaskt1m1_03_getRow('wh_assets', array('id'=>$details->item_id));  
                ...
                $html .= '<tr>
                            ...
                            <td width="10%" id="input_remarks"><input type="text" name="po_remarks[]" class="form-control"></td>
