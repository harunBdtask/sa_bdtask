...
use App\Modules\Machine\Models\Bdtaskt1ItemApproveModel;
use App\Models\Bdtaskt1m1CommonModel;
use App\Libraries\Permission;

class Bdtaskt1c2ItemApprove extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->bdtaskt1m12c13_01_item_approveModel = new Bdtaskt1ItemApproveModel();
    ...
    *--------------------------*/
    public function index()
    {
        ...
        $data['main_store_list']       = $this->bdtaskt1m12c13_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_material_store', array('status'=>1,'branch_id'=>session('branchId')));
        ...
        if($action=='approve'){
            ...
            for($i=1; $i<=$item_counter; $i++){
                ...
                if( $aqty >0 ){
                    ...
                    if($receive_type == 'rec'){
                        $stock_data = $this->bdtaskt1m12c13_02_CmModel->bdtaskt1m1_03_getRow('wh_material_receive_details', $where_check);
                    } else {
                        $stock_data = $this->bdtaskt1m12c13_02_CmModel->bdtaskt1m1_03_getRow('wh_material_transfer_details', $where_check);
                    }
            ...
            for($i=1; $i<=$item_counter; $i++){
                ...
                if($receive_type == 'rec'){
                    $receive_details = $this->bdtaskt1m12c13_02_CmModel->bdtaskt1m1_03_getRow('wh_material_receive_details', array('id'=>$receive_details_id));
                } else {
                    $receive_details = $this->bdtaskt1m12c13_02_CmModel->bdtaskt1m1_03_getRow('wh_material_transfer_details', array('id'=>$receive_details_id));
                }

                $where = array('request_id'=> $id,'item_id'=> $this->request->getVar('item_id'.$i) );
                $data2 = array(
                    'aqty'              => $this->request->getVar('aqty'.$i), 
                    'avail_qty'         => $this->request->getVar('aqty'.$i), 
    ...
    *--------------------------*/
    ...
    { 
        ...
        foreach($request_details as $details)
        {
            ...
            $stock_row = $this->bdtaskt1m12c13_02_CmModel->bdtaskt1m1_03_getRow('wh_material_stock', array('store_id'=>$main_store_id, 'item_id'=>$details->item_id ));
            $sub_stock_row = $this->bdtaskt1m12c13_02_CmModel->bdtaskt1m1_03_getRow('wh_machine_stock', array('sub_store_id'=>$request->sub_store_id, 'item_id'=>$details->item_id ));

            $batch_info = $this->bdtaskt1m12c13_01_item_approveModel->bdtaskt1m12_06_getItemStockDetailsById($details->item_id, $main_store_id);
            $batch_hidden = '';
            $batch_option = '<option value="" selected>Select Batch</option>';
            foreach($batch_info as $batch){
                $avail_qty = number_format($batch->avail_qty, 2, '.', '');
                if( $avail_qty <0 ){
                    $avail_qty = 0;
                }
            ...
            /*$batch_info = $this->bdtaskt1m12c13_01_item_approveModel->bdtaskt1m12_07_getStockTransferDetailsById($details->item_id, $main_store_id);
            foreach($batch_info as $batch){
                $avail_qty = number_format($batch->avail_qty, 2, '.', '');
                if( $avail_qty <0 ){
                    $avail_qty = 0;
                ...
                $batch_option .='<option value="tran_'.$batch->id.'">'.$batch->voucher_no.' > '.$batch->batch_no.' > '.$avail_qty.' '.(($batch->expiry_date =='')?'':(($today_date > $expiry_date_new)?'Expired':'Exp. '.$expiry_date)).'</option>';
                $batch_hidden .='<input type="hidden" id="receive_avail_qty_tran_'.$batch->id.'" value="'.$avail_qty.'">';
            ...
            $sub_stock_value = 0;
            if(!empty($sub_stock_row)){
                if($sub_stock_row->stock >0 ){
                    $sub_stock_value = $sub_stock_row->stock;
                }
            }
            ...
            $html .= '<tr>
                        ...
                        <td width="10%" align="right" class="valign-middle">'.$sub_stock_value.'</td>
                        ...
                        <td width="15%" class="valign-middle" align="right"><select name="receive_details_id'.$item_counter.'" id="receive_details_id'.$item_counter.'" onchange="change_avail_qty(this.value, '.$item_counter.')" class="form-control custom-select">'.$batch_option.'</select>'.$batch_hidden.'</td>
                        ...
                        <td width="15%"><input type="text" name="aqty'.$item_counter.'" id="aqty'.$item_counter.'" class="form-control text-right onlyNumber" value="'.$aqty.'" onkeyup="check_quantity_exceed('.$item_counter.')" ></td>
    ...
    *--------------------------*/
    ...
    { 
        ...
        foreach($request_details as $details)
        {
            ...
            $html .= '<tr>
                        <td width="30%">'.$item_row->nameE.' ('.$item_row->company_code.')<input type="hidden" name="item_id'.$item_counter.'" id="item_id'.$item_counter.'" value="'.$details->item_id.'"></td>
