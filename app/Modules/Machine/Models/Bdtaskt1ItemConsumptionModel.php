...
class Bdtaskt1ItemConsumptionModel extends Model
{
    ...
    public function bdtaskt1m12_02_getAll($postData=null){
        ...
        $builder3->select("wh_machine_requests.*");
        if($searchQuery != ''){
        ...
        foreach($records as $record ){ 
            ...
            if($record['isApproved']==1){
                $status = '<div class="badge badge-success" >'.get_phrases(['store','confirmed']).'</div>';
                ...
                $status = '<div class="badge badge-info text-white">'.get_phrases(['not','confirmed']).'</div>';
                
                $button .= (!$this->hasUpdateAccess)?'':'<a href="javascript:void(0);" class="btn btn-success-soft btnC actionApprove custool mr-2" title="'.$approve.'" data-id="'.$record['id'].'"><i class="fa fa-check"></i></a>';
    ...
    public function bdtaskt1m12_03_getItemRequestDetailsById($id){
        $data = $this->db->table('wh_machine_requests')->select('wh_machine_requests.*, DATE_FORMAT(wh_machine_requests.date, "%d/%m/%Y") date, IF(wh_machine_requests.isApproved=1,"Approved",IF(wh_machine_requests.isApproved=2,"Rejected","Not Approved")) as status_text, IF(wh_machine_requests.isCollected=1,"Collected","") as status_collected')
    ...
    public function bdtaskt1m12_10_getItemBatchById($item_id){
        $data = $this->db->table('wh_material_receive_details rd')->select('rd.id, rd.avail_qty, r.voucher_no')
            ->join('wh_material_receive r', 'r.id=rd.receive_id','left')
            ->where( array('rd.item_id'=>$item_id) )
            ->get()
            ->getResult();
