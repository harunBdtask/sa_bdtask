...
<div class="modal fade bd-example-modal-xl" id="ItemApproveDetails-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-xl" >
        <div class="modal-content">
            ...
            <?php echo form_open_multipart('machine/add_item_approve', 'class="ajaxForm needs-validation" id="ajaxForm" novalidate="" data="showCallBackData"');?>
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                        <div id="ItemApproveDetails_main_store_id"></div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() { 
       "use strict";
        ...
        $('#item_approveList').on('click', '.actionPreview', function(e){
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#ItemApproveDetails_main_store_id').text(data.main_store_name);
                    ...
                    $('#main_store_id').val(data.main_store_id);
        ...
        $('#item_approveList').on('click', '.actionCollect', function(e){
            e.preventDefault();
            $('.ajaxForm').removeClass('was-validated');
            //$('#thumbpic').next('span').text('');
            $('#item_details_preview').html('');
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('#ItemApproveDetails-modal').modal('show');
                    $('#ItemApproveDetailsModalLabel').text('<?php echo get_phrases(['collect','returned','item']);?>');
        ...
        $('#item_approveList').on('click', '.actionApprove', function(e){
            ...
            var id = $(this).attr('data-id');
            var submit_url = _baseURL+'machine/getItemApproveDetailsById/'+id;
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    get_item_details(id, data.main_store_id);
        ...
        $('#item_approveList').DataTable({ 
            ...
            'serverMethod': 'post',
            'ajax': {
               'url': _baseURL + 'machine/getItemApprove',
               'data':function ( d ) {
        ...
        $('#item_approveList').on('click', '.actionEdit', function(e){
            e.preventDefault();
            $('.ajaxForm').removeClass('was-validated');
            //$('#thumbpic').next('span').text('');
            $('#item_details_preview').html('');
            ...
            var id = $(this).attr('data-id');
            var submit_url = _baseURL+'machine/getItemApproveById/'+id;
            ...
            $.ajax({
                ...
                async: false,
                success: function(data) {
                    $('#item_approve-modal').modal('show');
                    $('#id').val(data.id);
                    $('#action').val('update');
                    $('#item_approveModalLabel').text('<?php echo get_phrases(['update', 'request']);?>');
        ...
        // delete item_approve
        $('#item_approveList').on('click', '.actionDelete', function(e){
            e.preventDefault();

            var id = $(this).attr('data-id');
            
            var submit_url = _baseURL+"machine/deleteItemApprove/"+id;
    ...
    function get_item_details_return(request_id){
        ...
        if(request_id !='' ){
            var submit_url = _baseURL+'machine/getItemCollectQuantityDetailsById';
    ...
    function get_item_details(request_id, main_store_id){

        if(request_id !='' ){
            var submit_url = _baseURL+'machine/getItemApproveQuantityDetailsById';
            ...
            $.ajax({
                ...
                url: submit_url,
                dataType : 'html',
                data: {'csrf_stream_name':csrf_val, 'request_id':request_id, 'main_store_id':main_store_id },
                async: false,
                success: function(data) {
                    $('#item_details_preview').html(data);
                    check_quantity();
    ...
    function carton_cal(sl){
        ...
        var carton = ($("#acarton"+sl).val()=='')?0:parseFloat($("#acarton"+sl).val());
        var carton_qty = ($("#carton_qty"+sl).val()=='')?0:parseFloat($("#carton_qty"+sl).val());
        ...
        if( carton > 0 && carton_qty >0 && box_qty >0 ){
            ...
            $("#aqty"+sl).val(total_qty);
            $("#aqty"+sl).prop('readonly', true);
            $("#abox"+sl).val(carton_qty);
            $("#abox"+sl).prop('readonly', true);
        } else if( carton_qty >0 && box_qty >0 ){
            var total_qty = carton_qty * box_qty;
            $("#aqty"+sl).val(total_qty);
            $("#aqty"+sl).prop('readonly', true);
            $("#abox"+sl).val(carton_qty);
            $("#abox"+sl).prop('readonly', false);
        } else {
            $("#aqty"+sl).val(qty);
            $("#aqty"+sl).prop('readonly', false);
            $("#abox"+sl).val('');
            $("#abox"+sl).prop('readonly', false);
        }
    }
    ...
    function box_cal(sl){
        ...
        var box = ($("#abox"+sl).val()=='')?0:parseInt($("#abox"+sl).val());
        var box_qty = ($("#box_qty"+sl).val()=='')?0:parseInt($("#box_qty"+sl).val());
        var qty = ($("#qty"+sl).val()=='')?0:parseFloat($("#qty"+sl).val());

        if( box > 0 ){
            var total_qty = box * box_qty;
            $("#aqty"+sl).val(total_qty);
            $("#aqty"+sl).prop('readonly', true);
        } else {
            $("#aqty"+sl).val(qty);
    ...
    function check_carton_exceed(sl){
        var req_qty = ($('#carton'+sl).val())?parseFloat($('#carton'+sl).val()):0;
        //var app_qty = $('#acarton'+sl).val();
        var aqty = ($('#acarton'+sl).val())?parseFloat($('#acarton'+sl).val()):0;
        //var avail_qty = parseFloat($('#avail_qty'+sl).val());
        //var min_qty = (avail_qty < req_qty)?avail_qty:req_qty;
        if(aqty > req_qty){
            toastr.warning('<?php echo get_notify('Carton_quantity_should_be_less_than_or_equal_to_request_carton_quantity'); ?>');
            $('#acarton'+sl).val(req_qty);
        }
        carton_cal(sl);
        check_quantity();
    }

    function check_box_exceed(sl){
        var req_qty = ($('#box'+sl).val())?parseFloat($('#box'+sl).val()):0;
        //var app_qty = $('#abox'+sl).val();
        var aqty = ($('#abox'+sl).val())?parseFloat($('#abox'+sl).val()):0;
        //var avail_qty = parseFloat($('#avail_qty'+sl).val());
        //var min_qty = (avail_qty < req_qty)?avail_qty:req_qty;
        if(aqty > req_qty){
            toastr.warning('<?php echo get_notify('Box_quantity_should_be_less_than_or_equal_to_request_box_quantity'); ?>');
            $('#abox'+sl).val(req_qty);
        }
        box_cal(sl);
        check_quantity();
    ...
    function check_quantity_exceed(sl){
        ...
        var min_qty = (avail_qty < req_qty)?avail_qty:req_qty;
        if(aqty > min_qty){
            toastr.warning('<?php echo get_notify('Quantity_should_be_less_than_or_equal_to_stock_and_request_quantity'); ?>');
            $('#aqty'+sl).val(min_qty);
        }
        check_quantity();
    }
    ...
    function check_quantity(){
        ...
        for(var i=1; i<=item_counter; i++){
            ...
            if(aqty >0 ){
                $('#receive_details_id'+i).attr('required','required');
            } else {
                $('#receive_details_id'+i).removeAttr('required');
            }
        }
