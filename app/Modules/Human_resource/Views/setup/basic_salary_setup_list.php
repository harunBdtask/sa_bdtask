<div class="row">
    <div class="col-sm-12">
        <?php if($permission->module('basic_salary_setup')->access()){ ?>
        <div class="card">
            <div class="card-header py-2">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-right">
                        <?php if($permission->method('basic_salary_setup', 'create')->access()){ ?>
                        <a href="javascript:void(0);" class="btn btn-success btn-sm mr-1 addBasicSalarySetupForm"><i class="fas fa-plus mr-1"></i><?php echo get_phrases(['add', 'basic', 'salary']);?></a>
                        <?php }?>
            ...
            <div class="card-body">
                <table id="basicSalarySetupList" class="table display table-bordered table-striped table-hover compact" width="100%">
                    <thead>
                        <tr>
                            <th><?php echo get_phrases(['serial']);?></th>
                            <th><?php echo get_phrases(['employee', 'type']);?></th>
                            <th><?php echo get_phrases(['percent']);?></th>
...
<div class="modal fade bd-example-modal-lg" id="add-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <?php echo form_open_multipart('human_resources/basic_salary_setup/basic_salary_create', 'class="needs-validation" id="basicSalaryForm" novalidate="" data="jobCallBackData"');?>
            <div class="modal-body">
                ...
                <div class="row">
                   <div class="col-md-12 col-sm-12">
                       <div class="form-group">
                            <label class="font-weight-600 mb-0"><?php echo get_phrases(['percent']);?> <i class="text-danger">*</i></label>
                             <input type="text" name="percent" id="percent" class="form-control onlyNumber" placeholder="<?php echo get_phrases(['percent']);?>" autocomplete="off" required>
                        </div>
                   </div>
                </div>
...
<script type="text/javascript">
    ...
    var jobCallBackData = function () { 
        ...
        $('#basicSalaryForm')[0].reset();       
        $('#basicSalaryForm').removeClass('was-validated');    
        // $('#basicSalarySetupList').DataTable().ajax.reload(null, false);
        location.reload();
    }
    ...
    $(document).ready(function() { 
        ...
        $('#basicSalarySetupList').DataTable({ 
             responsive: true,
             lengthChange: false,
             "aaSorting": [[ 1, "asc" ]],
             "columnDefs": [
            ...
            'serverMethod': 'post',
            'ajax': {
               'url': _baseURL + 'human_resources/basic_salary_setup/get_basic_salary_setup_list',
        ...
        $('.addBasicSalarySetupForm').on('click', function(){
            ...
            $('#addModalLabel').text('<?php echo get_phrases(['add', 'basic', 'salary', 'setup']);?>');
            $('#add-modal').modal('show');

        });


        $('#basicSalarySetupList').on('click', '.actionEdit', function(e){
            e.preventDefault();
            var id = $(this).attr('data-id');

            //console.log(id);

            var submit_url = _baseURL+"human_resources/basic_salary_setup/basic_salary_by_id/"+id; 
            $.ajax({
                type: 'POST',
                url: submit_url,
                data: {'csrf_stream_name':csrf_val},
                ...
                success: function(response) {
                    ...
                    $('#addModalLabel').text('<?php echo get_phrases(['update', 'basic', 'salary', 'setup']);?>');
                    $('#add-modal').modal('show');
        ...
        // delete department
        $('#basicSalarySetupList').on('click', '.actionDelete', function(e){
            e.preventDefault();

            var id = $(this).attr('data-id');
            
            var submit_url = _baseURL+"human_resources/basic_salary_setup/delete_basic_salary/"+id;
