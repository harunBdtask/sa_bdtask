            ...
            <div class="card-header py-2">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-right">
                        <?php if($permission->method('branch', 'create')->access()){ ?>
                        <a href="javascript:void(0);" class="btn btn-success btn-sm mr-1 addBranch"><i class="fas fa-plus mr-1"></i><?php echo get_phrases(['add', 'new', 'branch']);?></a>
                        <?php }?>
                ...
                <table id="branchList" class="table display table-bordered table-striped table-hover compact" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo get_phrases(['branch']);?></th>
...
<div class="modal fade bd-example-modal-lg" id="add-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <?php echo form_open_multipart('human_resources/branch/addBranch', 'class="needs-validation" id="branchForm" novalidate="" data="jobCallBackData"');?>
                ...
                <div class="row">
                   <div class="col-md-12 col-sm-12">
                       <div class="form-group">
                            <label class="font-weight-600 mb-0"><?php echo get_phrases(['branch', 'name']);?> <i class="text-danger">*</i></label>
                             <input type="text" name="name" id="name" class="form-control" placeholder="<?php echo get_phrases(['enter', 'branch', 'name']);?>" autocomplete="off" required>
                        </div>
                   </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() { 
        ...
        $('#branchList').DataTable({ 
            ...
            'serverMethod': 'post',
            'ajax': {
               'url': _baseURL + 'human_resources/branch/get_branch_list',
        ...
        $('.addBranch').on('click', function(){
            ...
            $('#saveBtn').text('<?php echo get_phrases(['add', 'branch']);?>');
            $('#addModalLabel').text('<?php echo get_phrases(['add', 'new', 'branch']);?>');
        ...
        $('#branchList').on('click', '.actionEdit', function(e){
            ...
            var submit_url = _baseURL+"human_resources/branch/getbranchById/"+id; 
            $.ajax({
                type: 'POST',
                url: submit_url,
                data: {'csrf_stream_name':csrf_val},
                ...
                success: function(response) {
                    ...
                    $('#action').val('update');
                    $('#saveBtn').text('<?php echo get_phrases(['update', 'branch']);?>');
                    $('#addModalLabel').text('<?php echo get_phrases(['update', 'branch', 'name']);?>');
                    $('#add-modal').modal('show');
                }
            });  
        });
        ...
        $('#branchList').on('click', '.actionDelete', function(e){
            ...
            var submit_url = _baseURL+"human_resources/branch/deleteBranchById/"+id;
