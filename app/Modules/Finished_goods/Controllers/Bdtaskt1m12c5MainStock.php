...
use App\Modules\Finished_goods\Models\Bdtaskt1m12MainStockModel;
use App\Models\Bdtaskt1m1CommonModel;
use App\Libraries\Permission;

class Bdtaskt1m12c5MainStock extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->bdtaskt1m12c5_01_main_stockModel = new Bdtaskt1m12MainStockModel();
    ...
    *--------------------------*/
    ...
    {
        $data['title']      = get_phrases(['production','store', 'balance']);
        $data['moduleTitle']= get_phrases(['finished','goods']);
        $data['isDTables']  = true;
        $data['module']     = "Finished_goods";
        $data['page']       = "main_stock/list";

        $data['hasPrintAccess']        = $this->permission->method('fg_stock', 'print')->access();
        $data['hasExportAccess']        = $this->permission->method('fg_stock', 'export')->access();
        
        $where = array('status'=>1,'branch_id'=>session('branchId') );

        $data['store_list']       = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_production_store', $where);
    ...
    *--------------------------*/
    ...
    { 
        ...
        if (! $this->validate($rules)) {
            ...
            if($action=='add'){
                $isExist = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_03_getRow('wh_production_stock', array('nameE'=>$this->request->getVar('nameE')));
                $isExist2 = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_03_getRow('wh_production_stock', array('nameA'=>$this->request->getVar('nameA')));
                if(!empty($isExist) || !empty($isExist2) ){
                    ...
                    //$data['image'] = $filePath;
                    $result = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_01_Insert('wh_production_stock',$data);
                    if($result){
                         $response = array(
                            'success'  =>true,
                ...
                $result = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_02_Update('wh_production_stock',$data, array('id'=>$id));
                if($result){
                     $response = array(
                        'success'  =>true,
    ...
    *--------------------------*/
    ...
    { 
        $data = $this->bdtaskt1m12c5_02_CmModel->bdtaskt1m1_03_getRow('wh_production_stock', array('id'=>$id));
        echo json_encode($data);
    }
