...
class Bdtaskt1m12CategoriesModel extends Model
{
    ...
    public function __construct()
    {
        ...
        $this->hasReadAccess = $this->permission->method('fg_categories', 'read')->access();
        //$this->hasCreateAccess = $this->permission->method('fg_categories', 'create')->access();
        $this->hasUpdateAccess = $this->permission->method('fg_categories', 'update')->access();
        $this->hasDeleteAccess = $this->permission->method('fg_categories', 'delete')->access();
    ...
    public function bdtaskt1m12_02_getAll($postData=null){
        ...
        $searchQuery = "";
        if($searchValue != ''){
           $searchQuery = " (wh_categories.nameE like '%".$searchValue."%' OR wh_categories.nameA like '%".$searchValue."%' ) ";
        }
        
        ## Fetch records
        $builder3 = $this->db->table('wh_categories');
        $builder3->select("wh_categories.*");
        if($searchValue != ''){
