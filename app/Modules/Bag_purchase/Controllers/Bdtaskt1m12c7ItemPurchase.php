...
use App\Modules\Bag_purchase\Models\Bdtaskt1m12ItemPurchaseModel;
...
class Bdtaskt1m12c7ItemPurchase extends BaseController
{
    ...
    *--------------------------*/
    public function index()
    {
        ...
        $data['store_list']     = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_bag_store', array('status'=>1));
        $data['supplier_list']  = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_bag_supplier_information', array('status'=>1));
        $data['all_po']  = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_bag_purchase', array('status'=>1));
    ...
    *--------------------------*/
    ...
    { 
        $data = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_06_Deleted('wh_bag', array('id'=>$id));
    ...
    *--------------------------*/
    ...
    { 
        ...
        if (! $this->validate($rules)) {
            ...
            if($action=='add'){
                $isExist = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_requisition', array('voucher_no'=>$this->request->getVar('voucher_no')));
                //$isExist2 = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_03_getRow('wh_bag', array('nameA'=>$this->request->getVar('nameA')));
                if( !empty($isExist)  ){
                    $response = array(
                            'success'  =>'exist',
                ...
                $result = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_02_Update('wh_bag_requisition',$data, array('id'=>$id));
                if($result){
                     $response = array(
                        'success'  =>true,
    ...
    *--------------------------*/
    ...
    { 
        $data = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_03_getRow('wh_bag', array('id'=>$id));
        echo json_encode($data);
    }
    ...
    *--------------------------*/
    ...
    { 
        ...
        {
            $item_row = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_03_getRow('wh_bag', array('id'=>$details->item_id));            
            $unit_row = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_03_getRow('list_data', array('id'=>$item_row->unit_id));
            $specification = $this->bdtaskt1m12c7_02_CmModel->bdtaskt1m1_03_getRow('list_data', array('id'=>$item_row->specification));

            $html .= '<tr>
                        <td width="30%">'.$item_row->nameE.' '.$specification->nameE.' Bag Size:'.$item_row->bag_size.' Liner Size:'.$item_row->liner_size.'</td>   
