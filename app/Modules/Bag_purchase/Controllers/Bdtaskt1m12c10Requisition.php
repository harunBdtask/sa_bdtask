...
use App\Modules\Bag_purchase\Models\Bdtaskt1m12RequisitionModel;
...
class Bdtaskt1m12c10Requisition extends BaseController
{
    ...
    *--------------------------*/
    public function index()
    {
        ...
        $data['spr_list']       = $this->bdtaskt1m12c10_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_bag_requisition', array('status'=>1));
    ...
    { 
        ...
        if ($quatation_details) {
            foreach($quatation_details as $details)
            {
                ...
                $supplier_row = $this->bdtaskt1m12c10_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_supplier_information', array('id'=>$details->supplier_id));   
                $purchase_details_row = $this->bdtaskt1m12c10_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_quatation_details', array('supplier_id'=>$details->supplier_id, 'item_id'=>$details->item_id, 'received_qty !='=>0));  
                $html .= '<tr>
                            <td width="5%">'.$sl.'</td>
                            ...
                            <td width="5%" align="right">'.(($purchase_details_row)?$purchase_details_row->price:0).'</td>
                        </tr>';
            }
            $html .= '</table>';
            ...
            if($action=='add'){
                ...
                if( !empty($isExist)  ){
                    ...
                    foreach($item_id as $key => $item){
                        $data2[] = array('purchase_id'=> $result,'item_id'=> $item, 'store_id'=> $store[$key], 'qty'=> $qty[$key], 'monthly_consumption'=> $monthly_consumption[$key], 'last_purchase_qnty'=> $last_purchase_qnty[$key], 'last_purchase_date'=> $last_purchase_date[$key], 'last_stock_qnty'=> $main_stock[$key] );

                    }
                    $result2 = $this->bdtaskt1m12c10_02_CmModel->bdtaskt1m1_01_Insert_Batch('wh_bag_requisition_details',$data2);

                    // Store log data
                    $this->bdtaskt1m12c10_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['purchase','order']), get_phrases(['created']), $result, 'wh_bag_requisition');
                ...
                $data2 = array();
                foreach($item_id as $key => $item){
                    $data2[] = array('purchase_id'=> $id,'item_id'=> $item, 'qty'=> $qty[$key], 'store_id'=> $store[$key], 'monthly_consumption'=> $monthly_consumption[$key], 'last_purchase_qnty'=> $last_purchase_qnty[$key], 'last_purchase_date'=> $last_purchase_date[$key], 'last_stock_qnty'=> $main_stock[$key] );
    ...
    *--------------------------*/
    ...
    { 
        ...
        foreach($item_list as $items){
            $html .='<option value="'.$items->id.'">'.$items->nameE.' '.$items->b_specification.' Bag Size:'.$items->bag_size.' Liner Size:'.$items->liner_size.'</option>';
        }
        echo $html;
   }
    /*--------------------------
    ...
    *--------------------------*/
    ...
    { 
        ...
        foreach($purchases_details as $details)
        {
            ...
            $html .= '<tr>
                        <td width="15%">'.$item_row->nameE.($specification->nameE?' '.$specification->nameE:' ').' Bag Size:'.$item_row->bag_size.' Liner Size:'.$item_row->liner_size.'</td>
