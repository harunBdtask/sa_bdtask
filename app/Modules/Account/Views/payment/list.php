...
<div class="row">
    <div class="col-sm-12">
        <?php 
        $hasPrintAccess  = $permission->method('payment_voucher', 'print')->access();
        $hasExportAccess = $permission->method('payment_voucher', 'export')->access();
        if($permission->method('payment_voucher', 'read')->access() || $permission->method('payment_voucher', 'update')->access()){ ?>
        <div class="card">
            <div class="card-header py-2">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-right">
                        <?php if($permission->method('payment_voucer', 'create')->access()){ ?>
                       <a href="<?php echo base_url('account/accounts/payment_voucher');?>" class="btn btn-success btn-sm mr-1"><i class="fas fa-plus mr-1"></i><?php echo get_phrases(['add', 'new', 'voucher']);?></a>
                        <?php }?>
                ...
                <table id="rvList" class="table display table-bordered table-striped table-hover compact" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo get_phrases(['receipt', 'voucher']);?></th>
...
<div class="modal fade bd-example-modal-lg" id="invoiceDetails-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-600" id="invoiceDetailsModalLabel"></h5>
            ...
            <div class="modal-body">
                <table class="table table-bordered table-striped">
                    <tr>                        
                        <td width="40%"><label for="invoiceDetails_doctor_name" class="font-weight-600"><?php echo get_phrases(['doctor', 'name']) ?> </label></td>
                        <td width="60%"><div id="invoiceDetails_doctor_name"></div></td>
                    </tr>
                    <tr>                        
                        <td><label for="invoiceDetails_patient_name" class="font-weight-600"><?php echo get_phrases(['patient', 'name']) ?> </label></td>
                        <td><div id="invoiceDetails_patient_name"></div></td>
                    </tr>                           
                </table>

                <div id="details_pricing"></div>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-danger" data-dismiss="modal"><?php echo get_phrases(['close']);?></button>
                <button type="button" class="btn btn-success" data-dismiss="modal"><?php echo get_phrases(['print']);?></button>
            </div>
...
<!-- journal voucher -->
<div class="modal fade bd-example-modal-lg" id="jv-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <div class="row">
                   ...
                   <div class="col-md-6 col-sm-12">
                       <div class="form-group">
                            ...
                            <input type="text" name="voucher_no" id="voucher_no" class="form-control" readonly="">
                        </div>
                   </div>
                   <div class="col-md-6 col-sm-12">
                       <div class="form-group">
                            ...
                            <table class="table table-stripped table-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center" width="20%"><?php echo get_phrases(['head', 'code']);?></th>
    ...
    $(document).ready(function() { 
       ...
       $('#search_date').datepicker({dateFormat: 'yy-mm-dd'});
        ...
        $('#rvList').DataTable({ 
            ...
            <?php if($hasPrintAccess){ ?>
                {
                    ...
                    className: 'btn-light',
                    title : 'Payment_Voucher_list<?php echo date('Y-m-d');?>',
                    exportOptions: {
            ...
            'serverMethod': 'post',
            'ajax': {
               'url': _baseURL + 'account/accounts/getPVList',
               'data': function ( d ) {
        ...
        // delete voucher 
        $('#rvList').on('click', '.jvAction', function(e){
            e.preventDefault();
            ...
            $('#service_div').html('');
            //loadFirstRow(1);
            var submit_url = _baseURL+"account/vouchers/maxVNo/JV"; 
            $.ajax({
                type: 'POST',
                url: submit_url,
                data: {'csrf_stream_name':csrf_val},
            ...
            if(CurDate >= addDate){
                ...
                if(check){ 
                    ...
                    $.ajax({
                        type: 'POST',
                        url: submit_url,
                        data: {'csrf_stream_name':csrf_val, patient_id:patient_id, voucher_id:voucher_id, action:'payment'},
                        dataType: 'JSON',
                        success: function(res) {
                            if(res.success==true){
                                toastr.success(res.message, res.title);
