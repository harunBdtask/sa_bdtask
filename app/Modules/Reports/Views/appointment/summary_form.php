<div class="row">
    <div class="col-sm-12">
        <?php 
        if($permission->method('appointment_summary_report', 'create')->access() || $permission->method('appointment_summary_report', 'read')->access()){ ?>
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-lg-3 col-md-3 col-sm-12">
                     ...
                     <?php if(session('user_role')==15 || session('user_role')==16){ ?>
                        <div class="col-lg-6 col-md-6 col-sm-12">
                            <div class="form-group">
                                <label for="doctor_id" class="col-form-label font-weight-600"><?php echo get_phrases(['doctor', 'name']) ?></label>
                                <?php echo form_dropdown('doctor_id[]','','','class="custom-select" id="assign_doctor_id" multiple');?>
                            </div>
                        </div>
                    <?php }else{?>
                        <div class="col-lg-6 col-md-6 col-sm-12">
...
<script type="text/javascript">
    ...
    $(document).ready(function() { 
       "use strict";
        ...
        if(nurseOrcoor=='15' || nurseOrcoor=='16'){
            var type = nurseOrcoor=='15'?'nurse':'coordinator';
            var user_id = '<?php echo session('id');?>';
            // doctor list
            $.ajax({
                type:'POST',
                url: _baseURL+'auth/getAssignDoctorList',
                dataType: 'json',
                data:{'csrf_stream_name':csrf_val, type:type, user_id:user_id},
            }).done(function(data) {
                $("#assign_doctor_id").select2({
        ...
        $('.filtering').on('click', function(e){
            if(nurseOrcoor=='15' || nurseOrcoor=='16'){
                var doctor_id = $('#assign_doctor_id').val();
            }else{
                var doctor_id = $('#doctor_id').val();
            }
            var date_range= $('#reportrange1').val();
            
            var submit_url = _baseURL+"reports/appointment/getSummaryReport";
            if(date_range !=''){
                preloader_ajax();
                $.ajax({
                    type: 'POST',
                    ...
                    success: function(res) {
                        $('#ex_doctor_id').val(doctor_id);
                        $('#ex_date_range').val(date_range);
