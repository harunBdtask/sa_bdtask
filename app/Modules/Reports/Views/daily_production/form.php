                ...
                <div class="row">
                    <div class="col-sm-2 text-right">
                        <div class="form-group">
                            <strong><?php echo get_phrases(['category']);?></strong> <i class="text-danger">*</i>
                        </div>
                    </div>
                    <div class="col-sm-3">
                        <div class="form-group">
                            <select name="cat_id" id="cat_id" class="form-control custom-select" required>
    ...
    $(document).ready(function() { 
       "use strict";
        ...
        $('.userIBtn').on('click', function(e){
            ...
            var cat_data = $('#cat_id').select2('data');
            var date = $('#date').val();
            if(cat_id =='' ){
            ...
            $.ajax({
                type: 'POST',
                url: submit_url,
                data: {'csrf_stream_name':csrf_val, cat_id:cat_id, date:date},
                dataType: 'JSON',
                success: function(res) {
                    $('#results').html('');
                    $('#results').html(res.data);
                    $('#title').text('Daily Production, Delivery/Stock Statement of '+cat_data[0].text);
