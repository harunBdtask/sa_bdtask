...
<div class="row">
    <div class="col-sm-12">
        <?php if($permission->method('doctor_com_report', 'create')->access() || $permission->method('doctor_com_report', 'read')->access()){ ?>
        <div class="card">
    ...
    $(document).ready(function() { 
       "use strict";
        ...
        $('.userIBtn').on('click', function(e){
            ...
            var submit_url = _baseURL+"reports/management/getDoctorCom"; 
            if(date){
                preloader_ajax();
                $.ajax({
                    type: 'POST',
        ...
        $(document).on('click', '.export', function(e){
            ...
            var service_id = $('#service_id').val();
            var date       = $('#reportrange1').val();
            var submit_url = _baseURL+"reports/management/exportExcel"; 
            if(date){
                preloader_ajax();
                $.ajax({
                    type: 'POST',
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(data.consumed_by == 'service'){
                        var invoiceNO = '<div data-id="'+data.invoice_id+'" data-type="SINV" class="clickable-row viewDetails text-success">SINV-'+data.invoice_id+' ('+data.invoice_date+')</div>'
                        $('.consumed_service_info').show();
                        $('#itemRequestDetails_invoice_id').html(invoiceNO);
