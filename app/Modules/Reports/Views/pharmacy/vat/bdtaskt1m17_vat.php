                ...
                <div class="row">
                    <div class="col-sm-3">
                        <div class="form-group">                           
                            <label class="font-weight-600"><?php echo get_phrases(['type']);?>  <i class="text-danger">*</i></label>
    ...
    $(document).ready(function() { 
       "use strict";
        ...
        $('.userIBtn').on('click', function(e){
            ...
            if(type ==''){
                toastr.warning('<?php echo get_notify('Select_type'); ?>');
                return
            }
            if(date_range =='' ){
            ...
            var submit_url = _baseURL+"reports/pharmacy/get_vat"; 
            $.ajax({
                type: 'POST',
                url: submit_url,
                data: {'csrf_stream_name':csrf_val, type:type, date_range:date_range},
                dataType: 'JSON',
                success: function(res) {
                    $('#results').html('');
                    $('#results').html(res.data);
                    $('#title').text('<?php echo get_phrases(['vat','report','date']);?> '+date_range);
