...
<script type="text/javascript">
    ...
    $(document).ready(function() { 
       "use strict";
        ...
        $('.userIBtn').on('click', function(e){
            ...
            var warehouse_data = $('#sub_store_id').select2('data');
    
            var submit_url = _baseURL+"reports/pharmacy/get_item_sub_stock"; 
            $.ajax({
                type: 'POST',
                url: submit_url,
                data: {'csrf_stream_name':csrf_val, sub_store_id:sub_store_id, item_id:item_id},
                dataType: 'JSON',
                success: function(res) {
                    $('#results').html('');
                    $('#results').html(res.data);
                    $('#title').text(warehouse_data[0].text+' <?php echo get_phrases(['available', 'stock']);?>');
    ...
    function preview(obj){
            ...
            var submit_url = _baseURL+'reports/pharmacy/getSubStoreItemReceiveDetails';
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#itemReceiveDetailsModalLabel').text('<?php echo get_phrases(['item','receive','details']);?>');
