                ...
                foreach ($results as $value) { 
                  ...
                  ?>
                   <tr onclick="onclick_change_bg('.detailsTable', this, 'cyan')">
                       ...
                       <td class="text-right"><?php echo esc(number_format($value->in_qty+$value->rec_qty, 2));?></td>
                       <td class="text-right"><?php echo esc(number_format($value->trans_qty, 2));?></td>
                       <td class="text-right"><a href="#" onclick="preview(this)" data-id="<?php echo esc($value->item_id);?>" warehouse-id="<?php echo esc($value->sub_store_id);?>"><?php echo esc(number_format($value->stock, 2));?></a></td>
                       <td><?php echo esc($value->unit_name);?></td>
                       <td class="text-right"><?php echo esc(number_format($value->price, 2));?></td>
                       <td class="text-right"><a href="#" onclick="preview(this)" data-id="<?php echo esc($value->item_id);?>" warehouse-id="<?php echo esc($value->sub_store_id);?>"><?php echo esc(number_format($value->stock*$value->price, 2));?></a></td>
                   </tr>
            <?php } 
          } else { ?>
                <tr>
