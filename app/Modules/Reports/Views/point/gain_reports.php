<div class="row">
    ...
    <div class="col-4 text-right mt-4">
        <h5><center><?php echo get_phrases(['gain', 'points', 'report']);?></center></h5>
    ...
    <table class="table table-bordered table-sm table-hover">
        <thead>
            <tr>
                ...
                <th width="8%"><?php echo get_phrases(['gain', 'points']);?></th>
                <th width="10%"><?php echo get_phrases(['value', 'of','amount']);?></th>
                <th width="14%"><?php echo get_phrases(['expiry', 'date', 'of', 'this', 'points']);?></th>
            </tr>
        </thead>
        <tbody>
            <?php
            if(!empty($reports)){
                ...
                ?>
                    <tr>
                        ...
                        <td><?php echo esc($value->gain_points);?></td>
                        <td><?php echo number_format($amount, 2);?></td>
                        <td><?php echo date('F j, Y', strtotime('-29 day', strtotime($value->invoice_date)));?></td>
                    </tr>
            <?php } }else{ ?>
                <tr>
...
<div class="no-print">
  ...
  <?php } if($export_access){ ?>
    <a href="<?php echo base_url('reports/pointing/exportGain?doctor_id='.$doctor_id.'&patient_id='.$patient_id.'&service_id='.$service_id.'&column_name='.$column_name.'&sorting='.$sorting.'&from='.$from.'&to='.$to);?>" class="btn btn-purple"><span class="fa fa-download"></span> <?php echo get_phrases(['export','excel']);?></a>
  <?php }?>
</div>