    ...
    <table class="table table-stripped table-sm table-hover detailsTable">
        <thead>
            <tr>
                ...
                <th width="10%"><?php echo get_phrases(['return','date']);?></th>
                <th width="15%"><?php echo get_phrases(['store']);?></th>
                <th width="10%"><?php echo get_phrases(['voucher','no']);?></th>
                <th width="15%"><?php echo get_phrases(['department']);?></th>
                <th width="5%"><?php echo get_phrases(['file','no']);?></th>
                ...
                <th width="10%" class="text-right"><?php echo get_phrases(['total','cost']);?></th>
            </tr>
        </thead>
        <tbody>
           <?php 
           ...
           if(!empty($results)){
                foreach ($results as $value) { 
                  $total += $value->return_qty*$value->price;
                  ...
                  ?>
                   <tr onclick="onclick_change_bg('.detailsTable', this, 'cyan')">
                       ...
                       <td><?php echo esc($value->returned_date);?></td>
                       ...
                       <td class="text-right"><a href="#" onclick="preview(this)" data-id="<?php echo esc($value->id);?>"><?php echo esc(number_format($value->return_qty, 2));?></a></td>
                       <td><?php echo esc($value->unit_name);?></td>
                       <td class="text-right"><?php echo esc(number_format($value->price, 2));?></td>
                       <td class="text-right"><a href="#" onclick="preview(this)" data-id="<?php echo esc($value->id);?>"><?php echo esc(number_format($value->return_qty*$value->price, 2));?></a></td>
                   </tr>
            <?php } 
          } else { ?>
                <tr>
