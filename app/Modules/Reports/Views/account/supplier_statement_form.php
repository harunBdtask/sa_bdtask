<div class="row">
    <div class="col-sm-12">
        <?php if($permission->method('supplier_statement', 'create')->access() || $permission->method('supplier_statement', 'read')->access()){ ?>
        <div class="card">
                    ...
                    <div class="col-md-8 col-sm-12">
                        <div class="row">
                            <div class="col-md-6 col-sm-12">
                                <div class="form-group">
                                    <label for="supplier_id" class="col-form-label-custom font-weight-600"><?php echo get_phrases(['supplier', 'name']) ?> <i class="text-danger">*</i></label>
                                    <?php echo form_dropdown('supplier_id','','','class="custom-select" id="supplier_id" required="required"');?>
                                </div>
                            </div>
                            <div class="col-md-4 col-sm-12">
                                <div class="form-group">
                                    <label for="supplier_date" class="col-form-label-custom font-weight-600"><?php echo get_phrases(['date', 'range']) ?> <i class="text-danger">*</i></label>
                                    <input type="text" name="supplier_date" id="supplier_date" class="form-control reportrange1" autocomplete="off" placeholder="<?php echo get_phrases(['select', 'date']);?>" required>
                                </div>
                            </div>
                            <div class="col-md-2 col-sm-12">
                                <div class="form-group mt-3 py-1">
                                    <button type="submit" class="btn btn-sm btn-success rounded-pill mt-1 supplierBtn"><?php echo get_phrases(['filter']);?></button>
                                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() { 
       "use strict";
        ...
        // search employee
        $('#supplier_id').select2({
            placeholder: '<?php echo get_phrases(['search', 'supplier']);?>',
            minimumInputLength: 3,
                ajax: {
                    url: _baseURL+'reports/account/searchSupplierAcc',
                    dataType: 'json',
                    delay: 250,
            ...
            var user_id = dat.id; 
            var txt = (dat.text).split('-');
            $('#full_name').val($.trim(txt[1]));
            $('#account_number').val(user_id);
            $('#account_name').val($.trim(txt[1]));
        });


        // $('#supplier_id').on('change', function(e){
        //     var user_id = $(this).val();
        //     var submit_url = _baseURL+"reports/account/getSupplierInfoById"; 
        //     $.ajax({
        //         type: 'POST',
        //         url: submit_url,
        //         data: {'csrf_stream_name':csrf_val, user_id:user_id},
        //         dataType: 'JSON',
        //         success: function(res) {
        //             $('#full_name').val(res.nameE);
        //             $('#account_number').val(res.acc_head);
        ...
        $('.supplierBtn').on('click', function(e){
            e.preventDefault();
            ...
            if(userId && date){
                ...
                $.ajax({
                    type: 'POST',
                    url: submit_url,
                    data: {'csrf_stream_name':csrf_val, branch_id:branchId, user_id:userId, date_range:date, type:'supplier'},
                    dataType: 'JSON',
                    success: function(res) {
                        $('#results').html('');
