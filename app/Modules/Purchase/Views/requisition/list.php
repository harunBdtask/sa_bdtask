...
<div class="modal fade bd-example-modal-xl" id="quotion_cs-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-xl custom-modal-dialog" >
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-600" id="quotion_csModalLabel"></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div> 
...
<script type="text/javascript">
    ...
    $(document).ready(function() { 
        ...
        $('body').on('click', '.addRow', function() {
            ...
            var html = '<tr>'+
                            ...
                            '<td class="valign-middle"><span id="where_store'+item_counter+'"></span></td>'+
        ...
        $('.addShowModal').on('click', function(){
            ...
            var html = '<tr>'+
                            ...
                            '<td class="valign-middle"><span id="where_store1"></span></td>'+
                            '<td><input type="hidden" name="existing[]" id="existing1" value="0"><button type="button" class="btn btn-success addRow" ><i class="fa fa-plus"></i></button></td>'+
                            '<input type="hidden" name="store[]" id="store1" />'+
                        '</tr>';      
            $("#po_item_div").html(html); 
        ...
        $('#purchase_orderList').on('click', '.quotationCSModal', function(e){
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#quotion_csModalLabel').text('<?php echo get_phrases(['quotion', 'CS']);?>');
                    $('#quotion_cs-modal').modal('show');
        ...
        // $('#po_supplier_id').on('select2:select', function () {
        $('#po_supplier_id').on('change', function () {
            var supplier_id = $(this).val();
            $('#supplier_id').val(supplier_id);
            var action = $('#po_action').val();
            if(action =='add' ){
                get_item_list(supplier_id);
            }
        }); 
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data.received == 1){
                        ...
                        // $('#po_supplier_id').val(data.supplier_id).trigger('change');
                        // $('#po_supplier_id').attr('disabled','disabled');
        ...
        $('#purchase_orderList').on('click', '.actionDelete', function(e){
            ...
            var submit_url = _baseURL+"purchase/deleteRequisition/"+id;
    ...
    function get_item_details_edit(purchase_id){
        ...
        if(purchase_id !='' ){
            var submit_url = _baseURL+'purchase/getRequisitionItemDetailsById';
            preloader_ajax();
            $.ajax({
                type: 'POST',
                url: submit_url,
                ...
                success: function(data) {
                    if(data != null){
                        ...
                        $.each(data, function(index, value){
                            ...
                            $("#where_store"+item_counter).text(value.store_name);
