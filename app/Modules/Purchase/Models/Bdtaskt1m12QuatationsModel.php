...
class Bdtaskt1m12QuatationsModel extends Model
{
    ...
    public function __construct()
    {
        ...
        $this->hasReadAccess = $this->permission->method('wh_material_quatation', 'read')->access();
        $this->hasCreateAccess = $this->permission->method('wh_material_quatation', 'create')->access();
        $this->hasUpdateAccess = $this->permission->method('wh_material_quatation', 'update')->access();
        $this->hasDeleteAccess = $this->permission->method('wh_material_quatation', 'delete')->access();

    }

    public function bdtaskt1m12_03_getPurchaseOrderDetailsById($id){
        $data = $this->db->table('wh_quatation')->select('wh_quatation.*, DATE_FORMAT(wh_quatation.date, "%d/%m/%Y") as date, spr.id as spr_id, spr.voucher_no as spr_no, 
            s.nameE as s_name, s.code_no as s_code')                        
            ->join('wh_material_requisition spr', 'spr.id=wh_quatation.requisition_id','left')    
            ->join('wh_supplier_information s', 's.id=wh_quatation.supplier_id','left')
            ->where( array('wh_quatation.id'=>$id) )
            ->get()
            ->getRowArray();
        return $data;
    }
    ...
    public function bdtaskt1m12_02_getAll($postData=null){
        ...
        if($date != ''){
           ...
           $searchQuery .= " ( wh_quatation.date = '".$date."' ) ";
        }
        ...
        if($spr != ''){
           ...
           $searchQuery .= " ( wh_quatation.requisition_id = '".$spr."' ) ";
        }
        
        ## Fetch records
        $builder3 = $this->db->table('wh_quatation');
        $builder3->select("wh_quatation.*, mr.voucher_no, mr.isApproved as spr_isApproved, si.nameE as s_nameE, si.code_no");
        $builder3->join('wh_material_requisition mr', 'mr.id=wh_quatation.requisition_id','left');
        $builder3->join('wh_supplier_information si', 'si.id=wh_quatation.supplier_id','left');
        // $builder3->join('wh_material_purchase mp', 'mp.quatation_id=wh_quatation.id','left');
        if($searchQuery != ''){
        ...
        foreach($records as $record ){ 
            $button = '';
            $spr_status = ' <div class="badge badge-success" >'.get_phrases(['approved']).'</div>';
            ...
            if ($record['isApproved'] == 0) {
                ...
                $status = ' <div class="badge badge-warning" >'.get_phrases(['partialy', 'purchased']).'</div>';
            }else{
                $status = ' ';
            }
            $data[] = array( 
    ...
    public function bdtaskt1m12_03_getCategoryDetailsById($id){
        $data = $this->db->table('wh_quatation')
                        ->select("wh_quatation.*, mr.voucher_no, si.nameE as s_nameE, si.code_no")
                        ->join('wh_material_requisition mr', 'mr.id=wh_quatation.requisition_id','left')
                        ->join('wh_supplier_information si', 'si.id=wh_quatation.supplier_id','left')
                        ->where( array('wh_quatation.id'=>$id) )
                        ->get()
                        ->getRowArray();
