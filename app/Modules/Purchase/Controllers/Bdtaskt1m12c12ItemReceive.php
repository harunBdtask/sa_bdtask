...
use App\Modules\Purchase\Models\Bdtaskt1m12ItemReceiveModel;
    ...
    *--------------------------*/
            ...
            if($action=='receive'){
                ...
                if( !empty($isExist)  ){
                    ...
                    if($result){
                        ...
                        $result4 = $this->bdtaskt1m12c12_02_CmModel->bdtaskt1m1_02_Update('wh_material_purchase',array('received'=>$received), array('id'=>$purchase_id));
                    ...
                    for($i=1; $i<=$item_counter; $i++){
                        ...
                        if ($quantity > 0) {
                            $data2[] = array('receive_id'=> $result,'store_id'=> $store_id,'purchase_id'=> $purchase_id,'item_id'=> $item_id, 'qty'=> $quantity, 'avail_qty'=> $quantity, 'price' => $price, 'total' => $total, 'vat_applicable' => $vat_applicable, 'vat_amount' => $vat_amount, 'remarks'=> $remarks, 'expiry_date'=> $expiry , 'batch_no'=> 'BTC-'.$item_id.$result );
                    ...
                    $result2 = $this->bdtaskt1m12c12_02_CmModel->bdtaskt1m1_01_Insert_Batch('wh_material_receive_details',$data2);
    ...
    public function bdtaskt1m16_09_supplierTransaction($payment_id, $supplier_id, $amount, $store_id)
    {
        ...
        $data = array(
            ...
            'Narration'   => 'Payment to Supplier, Name: '.$supplier_row->nameE.' Against MR-'. $payment_id,
        ...
        $data = array(
            ...
            'Narration'   => 'Amount in Store, Name: '.$store_row->nameE.' Against MR-'. $payment_id,
            'Debit'       => $debit,
            'Credit'      => $credit,
            'PatientID'   => 0,
            'BranchID'     => session('branchId'),
    ...
    *--------------------------*/
    ...
    public function bdtaskt1m16_09_supplierTransactionNew($payment_id, $supplier_id, $amount, $store_id)
    {
        ...
        $voucherdata = array(
            ...
            'subType'         => $sub_type,
            'subCode'         => $subcodes,
            'ledgerComment'   => 'Purchase Receive '.$payment_id,
            'Narration'       => 'Purchase Receive '.$payment_id,
            'isApproved'      => 0,
            'CreateBy'        => session('id'),
            'CreateDate'      => date('Y-m-d H:i:s'),
        );
