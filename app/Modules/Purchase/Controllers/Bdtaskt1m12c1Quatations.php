...
use App\Modules\Purchase\Models\Bdtaskt1m12QuatationsModel;
...
class Bdtaskt1m12c1Quatations extends BaseController
{
    ...
    *--------------------------*/
    public function index()
    {
        ...
        $data['hasCreateAccess']    = $this->permission->method('wh_material_quatation', 'create')->access();
        $data['hasPrintAccess']     = $this->permission->method('wh_material_quatation', 'print')->access();
        $data['hasExportAccess']    = $this->permission->method('wh_material_quatation', 'export')->access();

        $data['approved_requisition_list']    = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_material_requisition', array('isApproved'=>1));
        $data['requisition_list']    = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_material_requisition', array('received !='=>1, 'isApproved'=>1));
        $data['supplier_list']       = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_supplier_information', array('status'=>1));
    ...
    *--------------------------*/
    ...
    { 
        $info = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_quatation', array('id'=>$id, 'status'=>1));
        $items = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_material_purchase', array('quatation_id'=>$id, 'status'=>1 ));
        ...
        $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_06_Deleted('wh_material_quatation_details', array('quatation_id'=>$id));
        $data = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_06_Deleted('wh_quatation', array('id'=>$id));
        // Store log data
        $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['quotation','record']), get_phrases(['deleted']), $id, 'wh_quatation');
    ...
    { 
        ...
        if (! $this->validate($rules)) {
            ...
            if($action=='add'){
                ...
                $spr_status = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_material_requisition', array('id'=>$data['requisition_id'], 'received'=>1));
                if(!empty($spr_status)){
                    $response = array(
                        'success'  =>false,
                ...
                $isExist = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_quatation', array('created_date'=>$this->request->getVar('created_date')));
                if(!empty($isExist)){
                    $response = array(
                            'success'  =>'exist',
                    ...
                    $result = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_01_Insert('wh_quatation',$data);
                    ...
                    $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_01_Insert_Batch('wh_material_quatation_details',$data2);

                    // Store log data
                    $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['item','quatation']), get_phrases(['created']), $result, 'wh_quatation');
                    if($result){
                         $response = array(
                            'success'  =>true,
                ...
                $info = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_quatation', array('id'=>$id));
                ...
                $result = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_02_Update('wh_quatation',$data, array('id'=>$id));
                ...
                // Store log data
                $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['item','quatation']), get_phrases(['updated']), $id, 'wh_quatation');
                if($result){
                     $response = array(
                        'success'  =>true,
    ...
    *--------------------------*/
    ...
    { 
        $data = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_quatation', array('id'=>$id));
        echo json_encode($data);
    }
    ...
    *--------------------------*/
    ...
    public function bdtaskt1m12c10_05_get_quatation_details_by_id()
    {
        ...
        $data = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_material_quatation_details', array('quatation_id'=>$quatation_id ));
        echo json_encode($data);
    }
    ...
    { 
        ...
        $data = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRowArray('wh_material_requisition_details', array('item_id'=>$item_id, 'purchase_id'=>$requisition_id));
        $item_row = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_material', array('id'=>$item_id));  
    ...
    *--------------------------*/
    ...
    { 
        ...
        $html = '<table class="table table-bordered w-100"><tr><th>'.get_phrases(['SL']).'</th><th>'.get_phrases(['raw', 'material', 'name']).'</th><th class="text-right">'.get_phrases(['request','quantity']).'</th><th class="text-center">'.get_phrases(['unit']).'</th><th class="text-right">'.get_phrases(['unit', 'price']).'</th><th  class="text-right">'.get_phrases(['total', 'price']).'</th></tr>';

        $purchases_details = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_material_quatation_details', array('requisition_id'=>$purchase_id, 'quatation_id'=>$quatation_id));
        $sl = 0;
        foreach($purchases_details as $details)
        {
            $sl++;
            $item_row = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_material', array('id'=>$details->item_id));   
    ...
    public function bdtaskt1m12c10_07_approvePurchaseOrder()
    {
        ...
        $quatation_id = $this->request->getVar('quatation_id');
        $data = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_material_requisition', array('id'=>$id, 'isApproved'=>0));
        ...
        $voucher_no     = 'PO-'.getMAXID('wh_material_purchase', 'id');
        $isExist = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_material_purchase', array('voucher_no'=>$voucher_no) );
        if( !empty($isExist)  ){
            $response = array(
                    'success'  =>'exist',
                    'message'  => get_phrases(['already', 'exists']),
            ...
            $result = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_01_Insert('wh_material_purchase',$data_1);

            $requisition_details = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_material_requisition_details', array('purchase_id'=>$data->id));
            ...
            $result2 = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_01_Insert_Batch('wh_material_purchase_details',$data2);

            // Store log data
            $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['purchase','order']), get_phrases(['created']), $result, 'wh_material_purchase');
        ...
        if (!empty(session('id'))) {
            ...
            $result = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_02_Update('wh_material_requisition',$data, array('id'=>$id));
            $result_q = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_02_Update('wh_quatation',array('isApproved'=>1), array('id'=>$quatation_id));
        }
        // Store log data
        $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['purchase','order']), get_phrases(['approved']), $id, 'wh_material_requisition');
    ...
    { 
        ...
        $data = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_material_requisition', array('id'=>$id));
        echo json_encode($data);
    }
