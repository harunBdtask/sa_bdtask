...
<div class="modal fade bd-example-modal-xl" id="item_receive-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-xl custom-modal-dialog" >
        <div class="modal-content">
           ...
           <?php echo form_open_multipart('lc/add_item_receive', 'class="ajaxForm needs-validation" id="ajaxForm" novalidate="" data="showCallBackData"');?>
            <div class="modal-body">
                ...
                <div class="row form-group">
                    ...
                    <label for="" class="col-sm-2 text-right font-weight-600"><?php echo get_phrases(['LC','shipment'])?>: </label>
                    <div class="col-sm-4">
                        <?php echo form_dropdown('shipment_id', '', '', 'class="custom-select" id="shipment_id" ');?>
                    </div>
                </div>
                <div class="row form-group">
                ...
                <div class="row form-group">
                    ...
                    <label for="qc_attachment" class="col-sm-2 col-form-label font-weight-600"><?php echo get_phrases(['QC','attachment'])?> <i class="text-danger">*</i></label>
                    <div class="col-sm-4">
                        <input name="qc_attachment" type="file" class="form-control" id="qc_attachment" required >
                        <small id="attachmentHelp" class="form-text text-muted">Allow file types: jpg, jpeg, png, gif, pdf</small>
                    </div>
                </div>
                ...
                <div class="row mt-2">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th width="10%" class="text-center"><?php echo get_phrases(['expiry', 'date'])?></th>
...
<script type="text/javascript">
    ...
    function getItemReceiveById(id){
        ...
        $('#gr_no').val('');
        if (id) {
            var submit_url = _baseURL+'lc/getItemReceiveById/'+id;
            preloader_ajax();
            $.ajax({
                type: 'POST',
                url: submit_url,
                ...
                success: function(data) {
                    ...
                    $('#supplier_name').html(data.supplier_name+' - '+data.supplier_code);
    ...
    function get_shipment_costs(lc_id) {
        preloader_ajax();
        $.ajax({
            url: _baseURL+"lc/select_shipment",
            type: 'POST',
            data: {'csrf_stream_name':csrf_val, 'lc_id':lc_id},
            dataType:"json",
            async: true,
            success: function (data) {
                $('#shipment_id option:first-child').val('').trigger('change');
                $('#shipment_id').empty();
                $('#shipment_id').select2({
                ...
                var option = new Option('', '', true, true);
                $("#shipment_id").append(option).trigger('change');
    ...
    function get_purchase_details(purchase_id){
        ...
        if(purchase_id !='' ){
            var submit_url = _baseURL+'lc/getItemPurchaseDetailsById';
    ...
    $(document).ready(function() { 
        ...
        $('#item_receiveList').on('click', '.actionPreview', function(e){
            ...
            var id = $(this).attr('data-id');
            var submit_url = _baseURL+'lc/getItemReceiveDetailsById/'+id;
            preloader_ajax();
            $.ajax({
                type: 'POST',
                url: submit_url,
                ...
                success: function(data) {
                    ...
                    $('#itemPurchaseDetails_supplier_id').text(data.supplier_name+' - '+data.supplier_code);
        ...
        $('#item_receiveList').DataTable({ 
            ...
            'serverMethod': 'post',
            'ajax': {
               'url': _baseURL + 'lc/getItemReceive',
               'data':function ( d ) {
        ...
        $('#item_receiveList').on('click', '.actionDelete', function(e){
            ...
            var submit_url = _baseURL+"lc/deleteItemReceive/"+id;
    ...
    function get_item_details(purchase_id, supplier_id){
        ...
        if(purchase_id !='' ){
            var submit_url = _baseURL+'lc/getItemPurchasePricingDetailsById';
            preloader_ajax();
            $.ajax({
                type: 'POST',
                url: submit_url,
    ...
    function item_info(item_id,sl){
        preloader_ajax();
        $.ajax({
            url: _baseURL+"lc/getItemDetailsById/"+item_id,
            type: 'POST',
