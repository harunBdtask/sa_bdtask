...
class Bdtaskt1m12CategoriesModel extends Model
{
    ...
    public function __construct()
    {
        ...
        $this->hasReadAccess = $this->permission->method('wh_bag_categories', 'read')->access();
        //$this->hasCreateAccess = $this->permission->method('wh_bag_categories', 'create')->access();
        $this->hasUpdateAccess = $this->permission->method('wh_bag_categories', 'update')->access();
        $this->hasDeleteAccess = $this->permission->method('wh_bag_categories', 'delete')->access();
    ...
    public function bdtaskt1m12_02_getAll($postData=null){
        ...
        $searchQuery = "";
        if($searchValue != ''){
           $searchQuery = " (wh_bag_categories.nameE like '%".$searchValue."%' OR wh_bag_categories.nameA like '%".$searchValue."%' ) ";
        }
        
        ## Fetch records
        $builder3 = $this->db->table('wh_bag_categories');
        $builder3->select("wh_bag_categories.*");
        if($searchValue != ''){
    ...
    public function bdtaskt1m12_03_getCategoryDetailsById($id){
        $data = $this->db->table('wh_bag_categories')->select('wh_bag_categories.*')     
                        ->where( array('wh_bag_categories.id'=>$id) )
                        ->get()
                        ->getRowArray();
