<?php namespace App\Modules\Bag\Controllers;
use App\Modules\Bag\Views;
use CodeIgniter\Controller;
use App\Modules\Bag\Models\Bdtaskt1m12CategoriesModel;
...
class Bdtaskt1m12c1Categories extends BaseController
{
    ...
    *--------------------------*/
    public function index()
    {
        ...
        $data['hasCreateAccess']       = $this->permission->method('wh_bag_categories', 'create')->access();
        $data['hasPrintAccess']        = $this->permission->method('wh_bag_categories', 'print')->access();
        $data['hasExportAccess']       = $this->permission->method('wh_bag_categories', 'export')->access();
    ...
    *--------------------------*/
    ...
    { 
        ...
        $data = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_06_Deleted('wh_bag_categories', array('id'=>$id));
        // Store log data
        $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['item','category']), get_phrases(['deleted']), $id, 'wh_bag_categories');
    ...
    *--------------------------*/
    ...
    { 
        ...
        if (! $this->validate($rules)) {
            ...
            if($action=='add'){
                $isExist = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_categories', array('nameE'=>$this->request->getVar('nameE')));
                if(!empty($isExist)){
                    $response = array(
                            'success'  =>'exist',
                    ...
                    $result = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_01_Insert('wh_bag_categories',$data);
                    // Store log data
                    $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['item','category']), get_phrases(['created']), $result, 'wh_bag_categories');
                    if($result){
                         $response = array(
                            'success'  =>true,
                ...
                $result = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_02_Update('wh_bag_categories',$data, array('id'=>$id));
                // Store log data
                $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_22_addActivityLog(get_phrases(['item','category']), get_phrases(['updated']), $id, 'wh_bag_categories');
                if($result){
                     $response = array(
                        'success'  =>true,
    ...
    *--------------------------*/
    ...
    { 
        $data = $this->bdtaskt1m12c1_02_CmModel->bdtaskt1m1_03_getRow('wh_bag_categories', array('id'=>$id));
        echo json_encode($data);
    }
