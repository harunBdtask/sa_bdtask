    ...
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header py-2">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-right">
                        ...
                        <button type="button" class="btn btn-success btn-sm mr-1 addShowModal"><i class="fas fa-plus mr-1"></i><?php echo get_phrases(['add', 'target', 'sales', 'commission']); ?></button>
            ...
            <div class="card-body">
                ...
                <table id="monthlyTargetCommission" class="table display table-bordered table-striped table-hover compact" width="100%">
                    <thead>
                        <tr>
                            <th><?php echo get_phrases(['sl']); ?></th>
                            <th><?php echo get_phrases(['dealer', 'name']); ?></th>
                            <th><?php echo get_phrases(['commission', 'month']); ?></th>
                            <th><?php echo get_phrases(['total', 'kg']); ?></th>
                            <th><?php echo get_phrases(['total', 'ton']); ?></th>
                            <th><?php echo get_phrases(['reached', 'target']); ?></th>
...
<div class="modal fade bd-example-modal-xl" id="commission-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel3" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <?php echo form_open_multipart('commission/save_dealer_target_commission', 'class="ajaxForm needs-validation" id="ajaxForm" novalidate="" data="showCallBackData"'); ?>
            <div class="modal-body">
                ...
                <div class="row form-group">
                    <label for="target_end" class="col-sm-2 col-form-label font-weight-600"><?php echo get_phrases(['sold', 'quantity']) . '(' . get_phrases(['ton']) . ')' ?> <i class="text-danger">*</i></label>
                    <div class="col-sm-8">
                        <input type="text" name="sold_qty_ton" placeholder="<?php echo get_phrases(['sold', 'quantity']) . '(' . get_phrases(['ton']) . ')'; ?>" class="form-control valid_number" id="sold_qty_ton" required readonly>
                    </div>
                </div>

                <div class="row form-group">
                    <label for="reached_target" class="col-sm-2 col-form-label font-weight-600"><?php echo get_phrases(['reached', 'target', 'range']) ?> <i class="text-danger">*</i></label>
                    <div class="col-sm-8">
                        <input type="text" name="reached_target" placeholder="<?php echo get_phrases(['reached', 'target', 'range']); ?>" class="form-control valid_number" id="reached_target" required readonly>
                    </div>
                </div>
                ...
                <div class="row form-group">
                    <label for="commission_amount" class="col-sm-2 col-form-label font-weight-600"><?php echo get_phrases(['commission', 'amount',]) ?> <i class="text-danger">*</i></label>
                    <div class="col-sm-8">
...
<script type="text/javascript">
    function reload_table() {
        $('#monthlyTargetCommission').DataTable().ajax.reload();
    }
    ...
    var showCallBackData = function() {
        ...
        $('#monthlyTargetCommission').DataTable().ajax.reload(null, false);
        location.reload();
    }
    $(document).ready(function() {
        "use strict";

        $('#monthlyTargetCommission').DataTable({
            responsive: true,
            lengthChange: true,
            "aaSorting": [
                [9, "asc"]
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': _baseURL + 'commission/getdealer_target_sales_commission',
                'data': function(d) {
        ...
        $('.addShowModal').on('click', function() {
            ...
            $('#comments').val('');
            $('#commissionModalLabel').text('<?php echo get_phrases(['add', 'target', 'commission']); ?>');
            $('.modal_action').text('<?php echo get_phrases(['save']); ?>');
            $('#commission-modal').modal('show');

        });

        $('#monthlyTargetCommission').on('click', '.actionEdit', function(e) {
            e.preventDefault();
            $('.ajaxForm').removeClass('was-validated');
        ...
        // delete commission
        $('#monthlyTargetCommission').on('click', '.actionDelete', function(e) {
            e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (status == 1) {
                    ...
                    $("#reached_target").val(data.target_range);
                    $("#sold_qty_ton").val(data.wight_ton);
