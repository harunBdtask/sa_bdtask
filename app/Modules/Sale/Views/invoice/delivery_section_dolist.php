...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#doList').on('click', '.actionPreview', function(e) {
            ...
            var submit_url = _baseURL + 'sale/deliver_order/getdoDetailsFactrysection/' + id;
        ...
        var mydatatable = $('#doList').DataTable({
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': _baseURL + 'sale/deliver_order/getdeliverysectiondo',
                'data': {
                    'csrf_stream_name': csrf_val
                }
            },
    ...
    function do_DeliverySection_aprroval(do_id) {
        $.ajax({
            type: 'POST',
            url: _baseURL + 'sale/deliver_order/deliverysection_do_approval',
            dataType: 'JSON',
            data: {
                'do_id': do_id,
                'csrf_stream_name': csrf_val
    ...
    function delivery_order(do_id) {
        $.ajax({
            type: 'POST',
            url: _baseURL + 'sale/deliver_order/do_delivery',
            data: {
                'do_id': do_id,
                'csrf_stream_name': csrf_val
            },
            success: function(data) {
                $("#do_details_modal").html(data);
                $('#doDetails-modal').modal('show');
                $('#doDetailsModalLabel').text('<?php echo get_phrases(['dO', 'deliver']); ?>');
