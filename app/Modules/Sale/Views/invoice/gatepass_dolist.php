...
<div class="row">
    <div class="col-sm-12">
        <div class="card">
            ...
            <div class="card-body">
                <table id="gatepassList" class="table display table-bordered table-striped table-hover compact" width="100%">
                    <thead>
                        <tr>
...
<script type="text/javascript">
    var showCallBackData = function() {
        ...
        $('.ajaxForm')[0].reset();
        $('#do-modal').modal('hide');
        $('#gatepassList').DataTable().ajax.reload(null, false);
    }
    ...
    $(document).ready(function() {
        "use strict";
        $(".select2").select2();
        $('#gatepassList').on('click', '.actionPreview', function(e) {
            e.preventDefault();
            $('.ajaxForm').removeClass('was-validated');

            var id = $(this).attr('data-id');

            var submit_url = _baseURL + 'sale/deliver_order/getdoGatePassdata/' + id;
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#doDetailsModalLabel').text('<?php echo get_phrases(['gate', 'pass', 'print']); ?>');
        ...
        var mydatatable = $('#gatepassList').DataTable({
            responsive: true,
            lengthChange: true,
            "aaSorting": [
                [4, "desc"]
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': _baseURL + 'sale/deliver_order/getgate_pass',
                'data': {
                    'csrf_stream_name': csrf_val
                }
            },
    ...
    function gatePassInfo(delivery_id) {
        $.ajax({
            type: 'POST',
            url: _baseURL + 'sale/deliver_order/gatepass_form',
            data: {
            ...
            success: function(data) {
                $("#do_details_modal").html(data);
                $('#doDetails-modal').modal('show');
                $('#doDetailsModalLabel').text('<?php echo get_phrases(['gatepass', 'info']); ?>');
            },
            error: function() {
    ...
    function gatePassApprove(challan) {
        $.ajax({
            type: 'POST',
            url: _baseURL + 'sale/deliver_order/approve_gatepass',
            dataType: 'JSON',
            data: {
                'csrf_stream_name': csrf_val,
                challan: challan
