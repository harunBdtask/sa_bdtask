                ...
                <table id="doList" class="table display table-bordered table-striped table-hover compact" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo get_phrases(['payment', 'status']); ?></th>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#doList').on('click', '.actionPreview', function(e) {
            ...
            var submit_url = _baseURL + 'sale/deliver_order/getaccountdoDetailsById/' + id;
        ...
        var mydatatable = $('#doList').DataTable({
            ...
            buttons: [
                ...
                if ($hasExportAccess) { ?> {
                        ...
                        titleAttr: 'Excel',
                        className: 'btn-light',
                        title: 'do_List-<?php echo date('Y-m-d'); ?>',
                        exportOptions: {
                            columns: [0, 1, 2, 3, 4]
                        }
                    },
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': _baseURL + 'sale/deliver_order/getaccountsdo',
                'data': {
                    'csrf_stream_name': csrf_val
                }
            },
    ...
    function do_accounts_aprroval(do_id) {
        $.ajax({
            type: 'POST',
            url: _baseURL + 'sale/deliver_order/accounts_do_approval',
            dataType: 'JSON',
            data: {
                'do_id': do_id,
                'csrf_stream_name': csrf_val
    ...
    function do_accounts_paid(do_id) {
        var id = do_id;

        var submit_url = _baseURL + 'sale/deliver_order/dealer_payment/' + id;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#do_details_modal").html(data);
                $('#doDetails-modal').modal('show');
                $('#doDetailsModalLabel').text('<?php echo get_phrases(['dealer', 'receive']); ?>');
    ...
    function check_payamount() {
        var payable_amount = $("#payable_amount").val();
        var paid_amount = $("#paid_amount").val();

        if (parseFloat(paid_amount) > parseFloat(payable_amount)) {
            toastr.error('Paid Amount can not greater than Payable Amount');
            $("#paid_amount").val(0);
            $("#due_amount").val(0);
            $("#paid_amount").focus();
        ...
        var due_amount = (parseFloat(paid_amount) > 0 ? parseFloat(payable_amount) : 0) - (paid_amount ? parseFloat(paid_amount) : 0);
        $("#due_amount").val(due_amount);
    }

    function full_paid() {
        var payable_amount = $("#payable_amount").val();
        $("#paid_amount").val((payable_amount ? payable_amount : 0));
        $("#due_amount").val(0);
    }
</script>