                ...
                <table id="doList" class="table display table-bordered table-striped table-hover compact" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo get_phrases(['do', 'added', 'by']); ?></th>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#doList').on('click', '.actionPreview', function(e) {
            ...
            var submit_url = _baseURL + 'sale/deliver_order/getdoDetailsById/' + id;
        ...
        var mydatatable = $('#doList').DataTable({
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': _baseURL + 'sale/deliver_order/getsalesadmindo',
                'data': {
                    'csrf_stream_name': csrf_val
                }
            },
    ...
    function request_add_to_do(do_id) {
        var check = confirm('<?php echo get_phrases(['are', 'you', 'sure?']) ?>');
        if (check == true) {
            $.ajax({
                type: 'POST',
                url: _baseURL + 'sale/deliver_order/sales_admin_approve',
                dataType: 'JSON',
                data: {
                    'do_id': do_id,
                    'csrf_stream_name': csrf_val
    ...
    function edit_do(do_id) {
        $.ajax({
            type: 'POST',
            url: _baseURL + 'sale/deliver_order/do_edit',
            data: {
                'do_id': do_id,
                'csrf_stream_name': csrf_val
            },
            success: function(data) {
                $("#do_details_modal").html(data);
                $('#doDetails-modal').modal('show');
                $('#doDetailsModalLabel').text('<?php echo get_phrases(['dO', 'edit']); ?>');
            },
            error: function() {
