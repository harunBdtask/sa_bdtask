...
class Bdtaskt1ItemConsumptionModel extends Model
{
    ...
    public function __construct()
    {
        ...
        $this->hasReadAccess = $this->permission->method('approve_material_req', 'read')->access();
        //$this->hasCreateAccess = $this->permission->method('approve_material_req', 'create')->access();
        $this->hasUpdateAccess = $this->permission->method('approve_material_req', 'update')->access();
        $this->hasDeleteAccess = $this->permission->method('approve_material_req', 'delete')->access();
    ...
    public function bdtaskt1m12_02_getAll($postData=null){
        ...
        $searchQuery = "";
        if($searchValue != ''){
           $searchQuery = " (mr.voucher_no like '%".$searchValue."%' OR mr.date like '%".$searchValue."%' OR s.nameE like '%".$searchValue."%' ) ";
        }
        if($production_id != ''){
            if($searchQuery != ''){
                $searchQuery .= " AND ";
            }
           $searchQuery .= " ( mr.production_id = '".$production_id."' ) ";
        }
        if($sub_store_id != ''){
            if($searchQuery != ''){
                $searchQuery .= " AND ";
            }
           $searchQuery .= " ( mr.sub_store_id = '".$sub_store_id."' ) ";
        }
        if($voucher_no != ''){
            if($searchQuery != ''){
                $searchQuery .= " AND ";
            }
           $searchQuery .= " ( mr.id = '".$voucher_no."' ) ";
        }
        if($date != ''){
            if($searchQuery != ''){
                $searchQuery .= " AND ";
            }
           $searchQuery .= " ( mr.date = '".$date."' ) ";
        }
        
        ## Fetch records
        $builder3 = $this->db->table('wh_machine_requests mr');
        $builder3->select("mr.*, p.voucher_no as production_plan, s.nameE as sub_store_name");
        if($searchQuery != ''){
        ...
        $builder3->join('wh_production p', 'p.id=mr.production_id','left');
        $builder3->join('wh_machine_store s', 's.id=mr.sub_store_id','left');
    ...
    public function bdtaskt1m12_03_getItemApproveDetailsById($id){
        $data = $this->db->table('wh_machine_requests mr')->select('mr.*, DATE_FORMAT(mr.date, "%d/%m/%Y") date, p.voucher_no as production_plan, s.nameE as sub_store_name, e.first_name as request_by_name, 
            ...
            ->join('hrm_employees e', 'e.employee_id=mr.request_by','left')
    ...
    public function bdtaskt1m12_06_getItemStockDetailsById($item_id){
        $data = $this->db->table('wh_material_receive_details')->select('*')  
    ...
    public function bdtaskt1m12_07_decreaseReceiveStock($item_id, $req_qty){
        ...
        foreach($batch_info as $receive_details){
            if( $rem_qty >0 ){
                ...
                $this->db->table('wh_material_receive_details')->set('used_qty', 'used_qty+'.(($qty)?$qty:0), FALSE)->set('avail_qty', 'avail_qty-'.(($qty)?$qty:0), FALSE)->where($where)->update();

                $where2 = array('store_id'=> $receive_details->store_id, 'item_id'=> $item_id );
                $this->db->table('wh_material_stock')->set('stock', 'stock-'.(($qty)?$qty:0), FALSE)->set('stock_out', 'stock_out+'.(($qty)?$qty:0), FALSE)->where($where2)->update();
