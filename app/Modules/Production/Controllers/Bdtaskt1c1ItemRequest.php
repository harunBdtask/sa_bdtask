...
use App\Modules\Production\Models\Bdtaskt1ItemRequestModel;
...
class Bdtaskt1c1ItemRequest extends BaseController
{
    ...
    *--------------------------*/
    public function index()
    {
        ...
        $data['hasCreateAccess']        = $this->permission->method('material_request', 'create')->access();
        $data['hasPrintAccess']        = $this->permission->method('material_request', 'print')->access();
        $data['hasExportAccess']        = $this->permission->method('material_request', 'export')->access();
        
        $data['production_list']       = $this->bdtaskt1m12c9_02_CmModel->bdtaskt1m1_05_getResultWhere('wh_production', array('status'=>1, 'material_request'=>0), null, null, array('id','DESC'));
    ...
    *--------------------------*/
    ...
    { 
        ...
        $production = $this->bdtaskt1m12c9_02_CmModel->bdtaskt1m1_03_getRow('wh_production', array('id'=>$production_id ));
        ...
        if (! $this->validate($rules)) {
            ...
            if($action=='add'){
                ...
                // $isExist = $this->bdtaskt1m12c9_02_CmModel->bdtaskt1m1_03_getRow('wh_production', array('id'=>$production_id, 'material_request'=>1 ));
                $isExist = $this->bdtaskt1m12c9_02_CmModel->bdtaskt1m1_03_getRow('wh_machine_requests', array('production_id'=>$production_id));
                ...
                if( !empty($isExist)  ){
                    ...
                    if($result){
                        // $this->bdtaskt1m12c9_02_CmModel->bdtaskt1m1_02_Update('wh_production', array('material_request'=>1 ), array('id'=>$production_id));
                    }
                    $item_id    = $this->request->getVar('item_id');
                    $qty        = $this->request->getVar('qty');
                    $recipe_qty = $this->request->getVar('recipe_qty');
                    $wip_qty    = $this->request->getVar('wip_qty');

                    $data2 = array();
                    foreach($item_id as $key => $item){
                        $data2[] = array('request_id'=> $result, 'item_id'=> $item, 'qty'=> $qty[$key], 'recipe_qty'=> $recipe_qty[$key], 'wip_qty'=> $wip_qty[$key] );
                    }
    ...
    *--------------------------*/
    ...
    { 
        ...
        $html = '<table class="table table-bordered w-100"><tr><th class="text-center">'.get_phrases(['raw','material']).'</th><th class="text-center">'.get_phrases(['request','quantity']).'</th><th class="text-center">'.get_phrases(['approved','quantity']).'</th><th class="text-center">'.get_phrases(['unit']).'</th><th class="text-center">'.get_phrases(['batch']).'</th></tr>';
        ...
        foreach($request_details as $details)
        {
            ...
            $html .= '<tr>
                        ...
                        <td width="15%" align="right">'.(($details->aqty >0)?number_format($details->aqty, 2):'Not yet').'</td>
